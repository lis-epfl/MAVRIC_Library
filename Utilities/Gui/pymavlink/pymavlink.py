...
MAVLINK_MSG_ID_RADAR_TRACKED_TARGET = 150
MAVLINK_MSG_ID_RADAR_VELOCITY_HIST = 151
MAVLINK_MSG_ID_RADAR_RAW_DATA = 152
...
class MAVLink_radar_tracked_target_message(MAVLink_message):
        ...
        def __init__(self, time_boot_ms, sensor_id, target_id, velocity, amplitude, distance):
                ...
                self._fieldnames = ['time_boot_ms', 'sensor_id', 'target_id', 'velocity', 'amplitude', 'distance']
                self.time_boot_ms = time_boot_ms
                self.sensor_id = sensor_id
        ...
        def pack(self, mav):
                return MAVLink_message.pack(self, mav, 117, struct.pack('<IfffBB', self.time_boot_ms, self.velocity, self.amplitude, self.distance, self.sensor_id, self.target_id))
...
class MAVLink_radar_velocity_hist_message(MAVLink_message):
        ...
        def __init__(self, time_boot_ms, sensor_id, velocity):
                ...
                self._fieldnames = ['time_boot_ms', 'sensor_id', 'velocity']
                self.time_boot_ms = time_boot_ms
                self.sensor_id = sensor_id
                self.velocity = velocity

        def pack(self, mav):
                return MAVLink_message.pack(self, mav, 37, struct.pack('<IB64s', self.time_boot_ms, self.sensor_id, self.velocity))

class MAVLink_radar_raw_data_message(MAVLink_message):
        '''
        Raw sampled data from doppler radar
        '''
        def __init__(self, time_boot_ms, sensor_id, values):
                MAVLink_message.__init__(self, MAVLINK_MSG_ID_RADAR_RAW_DATA, 'RADAR_RAW_DATA')
                self._fieldnames = ['time_boot_ms', 'sensor_id', 'values']
                self.time_boot_ms = time_boot_ms
                self.sensor_id = sensor_id
                self.values = values

        def pack(self, mav):
                return MAVLink_message.pack(self, mav, 213, struct.pack('<I64hB', self.time_boot_ms, self.values, self.sensor_id))
...
mavlink_map = {
        ...
        MAVLINK_MSG_ID_RADAR_TRACKED_TARGET : ( '<IfffBB', MAVLink_radar_tracked_target_message, [0, 4, 5, 1, 2, 3], 117 ),
        MAVLINK_MSG_ID_RADAR_VELOCITY_HIST : ( '<IB64s', MAVLink_radar_velocity_hist_message, [0, 1, 2], 37 ),
        MAVLINK_MSG_ID_RADAR_RAW_DATA : ( '<I64hB', MAVLink_radar_raw_data_message, [0, 2, 1], 213 ),
...
class MAVLink(object):
        ...
        def radar_tracked_target_encode(self, time_boot_ms, sensor_id, target_id, velocity, amplitude, distance):
                ...
                time_boot_ms              : Timestamp (milliseconds since system boot) (uint32_t)
                sensor_id                 : Sensor ID (uint8_t)
                target_id                 : Temporary enumeration of target (uint8_t)
                velocity                  : Velocity estimate (float)
                amplitude                 : Amplitude (float)
                distance                  : Distance estimate (float)

                '''
                msg = MAVLink_radar_tracked_target_message(time_boot_ms, sensor_id, target_id, velocity, amplitude, distance)
                msg.pack(self)
                return msg
        ...
        def radar_tracked_target_send(self, time_boot_ms, sensor_id, target_id, velocity, amplitude, distance):
                ...
                '''
                return self.send(self.radar_tracked_target_encode(time_boot_ms, sensor_id, target_id, velocity, amplitude, distance))
        ...
        def radar_velocity_hist_encode(self, time_boot_ms, sensor_id, velocity):
                ...
                time_boot_ms              : Timestamp (milliseconds since system boot) (uint32_t)
                sensor_id                 : Sensor ID (uint8_t)
                velocity                  : Velocity field (uint8_t)

                '''
                msg = MAVLink_radar_velocity_hist_message(time_boot_ms, sensor_id, velocity)
                msg.pack(self)
                return msg
        ...
        def radar_velocity_hist_send(self, time_boot_ms, sensor_id, velocity):
                ...
                '''
                return self.send(self.radar_velocity_hist_encode(time_boot_ms, sensor_id, velocity))
            
        def radar_raw_data_encode(self, time_boot_ms, sensor_id, values):
                '''
                Raw sampled data from doppler radar
                ...
                '''
                msg = MAVLink_radar_raw_data_message(time_boot_ms, sensor_id, values)
                msg.pack(self)
                return msg
            
        def radar_raw_data_send(self, time_boot_ms, sensor_id, values):
                '''
                Raw sampled data from doppler radar
                ...
                '''
                return self.send(self.radar_raw_data_encode(time_boot_ms, sensor_id, values))
