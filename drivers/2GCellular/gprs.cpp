...
the MG2639. gprs, a friend class of MG2639_Cell, is defined with member
...
IPAddress gprs::localIP() // AT+ZIPGETIP
{
	int iRetVal;
...
int gprs::hostByName(const char * domain, IPAddress * ipRet) // AT+ZDNSGETIP
{
	int iRetVal;
...
int gprs::connect(const char * domain, unsigned int port, uint8_t channel)
{
	IPAddress destIP;
	hostByName(domain, &destIP);
	connect(destIP, port, channel);
}
int gprs::connectUDP(const char * domain, unsigned int port, uint8_t channel)
{
	IPAddress destIP;
	hostByName(domain, &destIP);
	connect(destIP, port, channel);
...
int gprs::connect(IPAddress ip, unsigned int port, uint8_t channel)
{
	int iRetVal;
...
int gprs::connectUDP(IPAddress ip, unsigned int port, uint8_t channel)
{
	int iRetVal;
	// Maximum is 15 for IP + 5 for port + 1 for channel + 12 for cmd, = and ,'s
	char ipSetupCmd[33];
	memset(ipSetupCmd, '\0', 33);
	sprintf(ipSetupCmd, "%s=%d,%d.%d.%d.%d,%d", UDP_SETUP, channel, ip[0], ip[1], ip[2], ip[3], port);
	//sprintf(ipSetupCmd, "%s=%i,%s,%i", TCP_SETUP, channel, ip, port);
	cell.sendATCommand((const char *)ipSetupCmd);
...
size_t gprs::write(uint8_t b)
{
	return write(&b, 1);
}

size_t gprs::writeUDP(uint8_t b)
{
	return write(&b, 1);
}

uint32_t gprs::write(const uint8_t *buf, uint32_t size)
{
	int iRetVal;
...
uint32_t gprs::writeUDP(const uint8_t *buf, uint32_t size)
{
	int iRetVal;
	// Maximum is 10 for command ',' and '=', 5 for port, 4 for length
	char sendCmd[19];
...
bool gprs::charToIPAddress(char * ipChar, IPAddress & ipRet)
{
	int a, b, c, d;
