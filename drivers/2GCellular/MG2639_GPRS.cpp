...
#include "../2GCellular/MG2639_GPRS.h"
...
int MG2639_GPRS::connectUDP(const char * domain, unsigned int port, uint8_t channel)
{
	IPAddress destIP;
	hostByName(domain, &destIP);
	connect(destIP, port, channel);
...
int MG2639_GPRS::connectUDP(IPAddress ip, unsigned int port, uint8_t channel)
{
	int iRetVal;
	// Maximum is 15 for IP + 5 for port + 1 for channel + 12 for cmd, = and ,'s
	char ipSetupCmd[33];
...
size_t MG2639_GPRS::writeUDP(uint8_t b)
{
	return write(&b, 1);
}
...
size_t MG2639_GPRS::writeUDP(const uint8_t *buf, size_t size)
{
	int iRetVal;
	// Maximum is 10 for command ',' and '=', 5 for port, 4 for length
	char sendCmd[19];
