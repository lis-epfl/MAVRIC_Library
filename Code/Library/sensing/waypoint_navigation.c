...
void receive_count(Mavlink_Received_t* rec, uint16_t* number_of_waypoints)
{
	...
	mavlink_msg_mission_request_send(MAVLINK_COMM_0,mavlink_system.sysid,mavlink_system.compid,waypoint_request_number);
}
...
//
		...
		if(packet.current == 2) {                                               //current = 2 is a flag to tell us this is a "guided mode" waypoint and not for the mission
			...
			mavlink_msg_mission_ack_send(MAVLINK_COMM_0, mavlink_system.sysid, mavlink_system.compid, 0);
			...
			if (waypoint_receiving){
				...
				if (packet.seq = waypoint_request_number)
				{
					...
					if (waypoint_request_number == number_of_waypoints) 
					{
						...
						mavlink_msg_mission_ack_send(MAVLINK_COMM_0, mavlink_system.sysid, mavlink_system.compid,type);
