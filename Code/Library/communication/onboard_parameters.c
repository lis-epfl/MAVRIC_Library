 ...
 */
static float onboard_parameters_read_parameter(onboard_parameter_set_t* param_set, int32_t param_index);
 ...
 */
static void onboard_parameters_update_parameter(onboard_parameter_set_t* param_set, int32_t param_index, float value);
...
void onboard_parameters_send_all_parameters_now(onboard_parameter_set_t* param_set) 
{
	for (uint8_t i = 0; i < param_set->param_count; i++)
	{
		mavlink_msg_param_value_send(MAVLINK_COMM_0,
										...
										onboard_parameters_read_parameter(param_set, i),
	...
	&& (uint8_t)set.target_component == (uint8_t)mavlink_system.compid) 
	{
		...
		for (uint16_t i = 0; i < param_set->param_count; i++) 
		{
			...
			if (match) 
			{
				...
				{
					onboard_parameters_update_parameter(param_set, i, set.param_value);
...
void onboard_parameters_read_parameters_from_flashc(onboard_parameter_set_t* param_set)
{
	...
	if ((param_set->param_count==local_array.values[0])&&(cksum1 == nvram_array->values[param_set->param_count + 1])&&(cksum2 == nvram_array->values[param_set->param_count + 2]))
	{
		...
		{
			onboard_parameters_update_parameter(param_set, i-1, local_array.values[i]);
		}
	}
	else
	{
...
void onboard_parameters_write_parameters_from_flashc(onboard_parameter_set_t* param_set)
{
	...
	{
		local_array.values[i] = onboard_parameters_read_parameter(param_set, i-1);
		cksum1 += local_array.values[i];
		cksum2 += cksum1;
	}
