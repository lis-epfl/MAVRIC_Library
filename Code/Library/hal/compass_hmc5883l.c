...
void compass_hmc58831l_update(compass_data_t *compass_outputs) 
{
	...
	twim_read(&AVR32_TWIM0, (uint8_t*)&(data[0]), 6, HMC5883_SLAVE_ADDRESS, false);
	
	compass_outputs->data[0] = (float)data[0];
	compass_outputs->data[1] = (float)data[1];
	compass_outputs->data[2] = (float)data[2];
}