		...
		case MAV_STATE_STANDBY:
			if (motor_switch == 1)
			{
				switch(channelSwitches)
				{
					case 0:
						...
						state_set_new_mode(&centralData->state_structure,MAV_MODE_ATTITUDE_CONTROL);
						break;
			...
			if (state_test_if_in_flag_mode(&centralData->state_structure,MAV_MODE_FLAG_SAFETY_ARMED))
			{
				switch (channelSwitches)
				{
					...
					case 1:
						state_set_new_mode(&centralData->state_structure,MAV_MODE_VELOCITY_CONTROL);
						if (centralData->waypoint_handler.in_the_air)
						{
							centralData->state_structure.mav_state = MAV_STATE_ACTIVE;
						}
...
{	
	...
	scheduler_add_task(scheduler    , ORCA_TIME_STEP_MILLIS * 1000.0f , RUN_REGULAR , PERIODIC_ABSOLUTE, PRIORITY_HIGH   , (task_function_t)&navigation_update                               , (task_argument_t)&centralData->navigationData		 , 3);
	scheduler_add_task(scheduler    , 200000                          , RUN_REGULAR , PERIODIC_ABSOLUTE, PRIORITY_NORMAL , &tasks_set_mav_mode_n_state                                       , 0                                                    , 4);
	scheduler_add_task(scheduler    , 4000                            , RUN_REGULAR , PERIODIC_ABSOLUTE, PRIORITY_NORMAL , (task_function_t)&mavlink_communication_update                    , (task_argument_t)&centralData->mavlink_communication , 5);
	scheduler_add_task(scheduler    , 100000                          , RUN_REGULAR , PERIODIC_ABSOLUTE, PRIORITY_LOW    , (task_function_t)&analog_monitor_update                           , (task_argument_t)&centralData->adc                   , 6);
	scheduler_add_task(scheduler    , 10000                           , RUN_REGULAR , PERIODIC_ABSOLUTE, PRIORITY_LOW    , (task_function_t)&waypoint_handler_control_time_out_waypoint_msg  , (task_argument_t)&centralData->waypoint_handler                                                    , 7);
