...
// Public definitions
//--------------------
>>>>>>> f54a137d589e6d9336066f72caf2be6c2dded4f9:Code/src/sensing/gps_aeropic.h
	...
	void _write_progstr_block(board_hardware_t *board, const prog_char *pstr, uint8_t size);
	static inline uint8_t pgm_read_byte(const void *s);
	void _send_progstr(board_hardware_t *board, const prog_char *pstr, uint8_t size);
...
inline int32_t GPS::_swapl(const void *bytes)
{
...
inline int16_t GPS::_swapi(const void *bytes)
{
	const uint8_t       *b = (const uint8_t *)bytes;
	union {
		int16_t v;
