...
const prog_char GPS_UBLOX::_ublox_set_binary[] PROGMEM = UBLOX_SET_BINARY;
const uint8_t GPS_UBLOX::_ublox_set_binary_size = sizeof(GPS_UBLOX::_ublox_set_binary);

board_hardware_t *board;

// Public Methods //////////////////////////////////////////////////////////////

void GPS_UBLOX::init(enum GPS_Engine_Setting nav_setting)
{
	board = get_board_hardware();
    // XXX it might make sense to send some CFG_MSG,CFG_NMEA messages to get the
    // right reporting configuration.

	//board->debug_stream.put(board->debug_stream.data,"uBlox nav_setting=%u\n", nav_setting);
	...
	reset:
        ...
        case 5:
            ...
            if (_payload_length > 512) {
                //board->debug_stream.put(board->debug_stream.data,"large payload %u");
				//board->debug_stream.put(board->debug_stream.data, (unsigned)_payload_length);
        ...
        case 7:
            ...
            if (_ck_a != data) {
				...
				board->debug_stream.put(board->debug_stream.data, "bad cka ");
				board->debug_stream.put(board->debug_stream.data, data);
				board->debug_stream.put(board->debug_stream.data, "should be ");
				board->debug_stream.put(board->debug_stream.data, _ck_a);
				board->debug_stream.put(board->debug_stream.data, "\n");
                _step = 0;
				goto reset;
            }
            break;
        case 8:
            ...
            if (_ck_b != data) {
				...
				board->debug_stream.put(board->debug_stream.data, "bad ckb ");
				board->debug_stream.put(board->debug_stream.data, data);
				board->debug_stream.put(board->debug_stream.data, "should be ");
				board->debug_stream.put(board->debug_stream.data, _ck_b);
				board->debug_stream.put(board->debug_stream.data, "\n");
                break;                                                  // bad checksum
            }
...
bool GPS_UBLOX::_parse_gps(void)
{
    if (_class == CLASS_ACK) {
        //Debug("ACK %u", (unsigned)_msg_id);
		board->debug_stream.put(board->debug_stream.data, _msg_id);
        return false;
    }

    if (_class == CLASS_CFG && _msg_id == MSG_CFG_NAV_SETTINGS) {
		board->debug_stream.put(board->debug_stream.data, "Got engine settings ");
		board->debug_stream.put(board->debug_stream.data, _buffer.nav_settings.dynModel);
		board->debug_stream.put(board->debug_stream.data, "\n");
    ...
    if (_class != CLASS_NAV) {
		...
		board->debug_stream.put(board->debug_stream.data, "Unexpected message 0x");
		board->debug_stream.put(board->debug_stream.data, _class);
		board->debug_stream.put(board->debug_stream.data, "02x 0x");
		board->debug_stream.put(board->debug_stream.data, _msg_id);
		board->debug_stream.put(board->debug_stream.data, "02x");
		board->debug_stream.put(board->debug_stream.data, "\n");
        if (++_disable_counter == 0) {
			...
			board->debug_stream.put(board->debug_stream.data, "Disabling message 0x");
			board->debug_stream.put(board->debug_stream.data, _class);
			board->debug_stream.put(board->debug_stream.data, "02x 0x");
			board->debug_stream.put(board->debug_stream.data, _msg_id);
			board->debug_stream.put(board->debug_stream.data, "02x");
    ...
    case MSG_POSLLH:
		...
		board->debug_stream.put(board->debug_stream.data, "MSG_POLLH next fix = ");
		board->debug_stream.put(board->debug_stream.data, next_fix);
		board->debug_stream.put(board->debug_stream.data, "\n");
    ...
    case MSG_STATUS:
		...
		board->debug_stream.put(board->debug_stream.data, "MSG_STATUS fix_status=");
		board->debug_stream.put(board->debug_stream.data, _buffer.status.fix_status);
		board->debug_stream.put(board->debug_stream.data, " fix_type =");
		board->debug_stream.put(board->debug_stream.data, _buffer.status.fix_type);
		board->debug_stream.put(board->debug_stream.data, "\n");
...
//               _buffer.solution.fix_type);
		board->debug_stream.put(board->debug_stream.data, "MSG_SOL fix status =");
		board->debug_stream.put(board->debug_stream.data, _buffer.solution.fix_status);
		board->debug_stream.put(board->debug_stream.data, " fix_type=");
		board->debug_stream.put(board->debug_stream.data, _buffer.solution.fix_type);
		board->debug_stream.put(board->debug_stream.data, "\n");
    ...
    case MSG_VELNED:
        //Debug("MSG_VELNED");
		board->debug_stream.put(board->debug_stream.data, "MSG_VELNED");
    ...
    default:
		...
		board->debug_stream.put(board->debug_stream.data, "Unexpected NAV message 0x");
		board->debug_stream.put(board->debug_stream.data, _msg_id);
		board->debug_stream.put(board->debug_stream.data, "02x\n");
		
        if (++_disable_counter == 0) {
            //Debug("Disabling NAV message 0x%02x", (unsigned)_msg_id);
			board->debug_stream.put(board->debug_stream.data, "Disabling NAV message 0x");
			board->debug_stream.put(board->debug_stream.data, _msg_id);
			board->debug_stream.put(board->debug_stream.data, "02x\n");
    ...
    if (_new_position && _new_speed) {
		...
		if (_fix_count == 100) {
			...
			board->debug_stream.put(board->debug_stream.data, "Asking for engine setting\n");
 ...
 */
void GPS_UBLOX::_update_checksum(uint8_t *data, uint8_t len, uint8_t &ck_a, uint8_t &ck_b)
{
    while (len--) {
        ck_a += *data;
        ck_b += ck_a;
 ...
 */
void GPS_UBLOX::_send_message(uint8_t msg_class, uint8_t msg_id, void *msg, uint8_t size)
{
 ...
 */
void GPS_UBLOX::_configure_message_rate(uint8_t msg_class, uint8_t msg_id, uint8_t rate)
{
 ...
 */
void GPS_UBLOX::_configure_gps(void)
{
...
/*
  ...
  matches a UBlox
 */
bool GPS_UBLOX::_detect(uint8_t data)
{
